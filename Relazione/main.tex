...
L’obiettivo principale consiste nel verificare le differenze tra i risultati ottenuti sui dataset originali e sui dataset sporcati. \\
Per fare ciò, analizziamo 3 dataset distinti, li sporcheremo e addestreremo 3 diversi modelli: in questo modo, potremo poi confrontare i vari risultati.

\subsection{Datasets}

Per questo progetto sono stati scelti tre dataset, reperibili su \hyperlink{https://www.kaggle.com/}{Kaggle}.\\
Questi hanno caratteristiche molto diverse:
\begin{enumerate}
...
\end{itemize}
    Come valore di \texttt{noise\_level}, dopo aver sperimentato con diversi valori, abbiamo scelto di usare 0.5.
...
Per contenere l'overfitting abbiamo introdotto tre meccanismi principali:
\begin{itemize}
...
La funzione \texttt{build\_model\_NN} si occupa della costruzione della rete neurale, definendo un'architettura composta da tre livelli densi con rispettivamente 32, 16 e 1 neurone. L'ultimo livello è il singolo neurone di output, e utilizza una funzione di attivazione sigmoide, poiché i dataset affrontano un problema di classificazione binaria e l'output rappresenta una probabilità compresa tra 0 e 1.\\
...
Il modello è stato configurato con il parametro \texttt{class\_weight = "balanced"}, al fine di compensare eventuali squilibri tra le classi e ridurre il rischio di bias verso la classe maggioritaria. Inoltre, è stato utilizzato \texttt{gamma = "scale"}, che imposta il valore di gamma a
\[
...
Il dataset vuole mostrare come il reddito annuo di un individuo dipende da diversi fattori. In modo intuitivo, è influenzato dal livello di istruzione, dall’età, dall’occupazione e così via.
...
In quanto ridondante con ‘educational-num’, abbiamo rimosso la feature ‘education’.\\
La distribuzione del target è: \textbf{76\% $\leq$ 50K, 24\% $>$ 50K}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth]{Immagini/Adult/DistribuzioneTargetAdult.png}
...
Inoltre, in media, sporcare le feature migliori porta a risultati peggiori rispetto allo sporcare feature peggiori.
...
Abbiamo mostrato i vari risultati ottenuti e fatto qualche confronto rilevante.
\par
...
Si potrebbero testare anche diverse modalità di applicazione del rumore, al fine di individuare strategie diverse.\\
